...
### 2026-07-23 simplicity reset

Dave's current Tipiti direction adds a stricter MVP filter:

> First make the business controllable: visible records, full CRUD, daily cockpit, and low-friction communications. Only deepen calendars, service logic, notifications, credits, inventory, and automation after the simple operating loop is useful.

This means workflow archetypes are still valid as internal grammar, but the near-term product surface should feel less like a platform architecture and more like:
...
Dave's revised review decision: OBA stays the source of truth. The separate `tipiti-cockpit` spike is now a UX/reference prototype only; the real implementation should be an OBA Today/Attention/Action layer with notifications/actions generated from OBA records and logged back into OBA.

See `docs/tipiti-stripped-down-simplicity.md` and `docs/oba-rescue-control-map-2026-07-23.md`.
